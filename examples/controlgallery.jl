...
function controlgallery()
	global spinbox, slider, progressbar
	...
	uiMenuItemOnClicked(item, cfunction(openClicked, Void, (Ptr{uiWindow},)), C_NULL);
	item = uiMenuAppendItem(menu, "Save");
	uiMenuItemOnClicked(item, cfunction(saveClicked, Void, (Ptr{uiWindow},)), C_NULL);
	item = uiMenuAppendQuitItem(menu);
	uiOnShouldQuit(cfunction(shouldQuit, Ptr{Void}, (Ptr{uiWindow},)), C_NULL);
	...
	uiSpinboxOnChanged(spinbox, cfunction(onSpinboxChanged, Void, (Ptr{uiSpinbox},)), C_NULL);
	uiBoxAppend(inner, uiControl_(spinbox), 0);

	slider = uiNewSlider(0, 100);
	uiSliderOnChanged(slider, cfunction(onSliderChanged, Void, (Ptr{uiSlider},)), C_NULL);
